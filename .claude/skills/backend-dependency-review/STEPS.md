...
**С отчётом architecture-review** — граница кода под ревью и блок Context берутся из его блока Scope, включая строку «базовая единица». Дособирай только то, что нужно для dependency-вердиктов и чего там нет: планируется ли замена конкретной технологии, сколько окружений, как в проекте принято подменять зависимости в тестах. Если строки «базовая единица» в отчёте нет — определи её сам по правилу ниже.

**Без отчёта** — определи код под ревью сам: diff или названные файлы плюс один шаг по графу зависимостей; модуль целиком плюс один шаг наружу; весь репозиторий — только если сказано явно, тогда стрелки считаются между модулями, а не между отдельными компонентами.

**Базовая единица** — обязательная строка блока Context: механизм, которым в этом проекте объявляют зависимость (параметр конструктора или функции, в том числе функция как значение; поле структуры; trait bound; `Protocol`; behaviour; `Context.Tag`; `Depends`; импорт модуля; получение из контейнера или реестра по ключу), и источник — где в коде это видно. Определяется по коду под ревью и соседним модулям, а не по тому, что принято в стеке. Тесты подтверждают выбор, если подменяют зависимости тем же механизмом; если не подменяют — единица всё равно берётся по объявлениям, рядом пишется «подмена в тестах: не используется». Строка не бывает «не известно»; при двух механизмах записываются оба и граница между ними. Для overengineering pass она задаёт нулевую точку стоимости indirection и ничего больше — см. [INVERSION.md](INVERSION.md).
...
Восстанови компоненты и стрелки с нуля. Название папки `domain/` или `ports/` ничего не доказывает: стрелка существует там, где один файл получает другой. Стрелка, причина которой в том, что в компоненте лежит работа другого слоя, уходит в «Вне scope» для `backend-architecture-review`; оценивается стрелка, которая останется после переноса, помеченная как условная.
...
- **Параметры конструкторов и функций** — что передаётся: конкретный клиент, contract или функция как значение; в DI-контейнере — что зарегистрировано и экспортировано (модули DI с `providers` и `exports`, `@Configuration` и `@Bean`, `services.Add*`, wiring контейнера); получение из контейнера или реестра по ключу.
...
Слой определяй по тому, с чем компонент работает напрямую, ровно шесть значений, те же, что у architecture-review: transport — HTTP/gRPC/queue handlers и DTO; application — сценарии и оркестрация; domain — правила без I/O; infrastructure — кэш, очереди, файлы, framework, конфигурация; persistence — БД, ORM, компоненты доступа к данным; external service — SDK и клиенты сторонних API.
...
Transport → Persistence — только при самостоятельной dependency-проблеме, см. ниже
Модуль → implementation detail другого модуля (его таблица, формат кэша, приватный тип)
```

**Transport → Persistence** не finding по факту существования. Если стрелка существует потому, что persistence-работа физически лежит в transport-компоненте (handler открывает транзакцию, строит запросы), это `backend-architecture-review`: в pipeline такая стрелка условная и оценивается после переноса; без отчёта место записывается в «Вне scope» с владельцем architecture-review, а оценивается стрелка, которая останется после переноса. Здесь стрелка оценивается, когда знание transport о persistence создаёт самостоятельную dependency-проблему: persistence-типы выходят в API transport; transport обязан знать поведение хранилища — транзакционность, ключи, порядок операций; несколько transport-компонентов зависят от одной implementation detail; изменение persistence требует согласованных правок в transport.

Для каждой ответь письменно:

1. **Consumer** — какой именно компонент зависит. Не слой, а конкретный компонент: класс, модуль, пакет, объявленный сервис.
...
Ошибки как канал протечки — не тема error handling. Здесь вопрос только один: **знает ли верхний слой тип ошибки нижнего**. Теряется ли контекст, проглатывается ли ошибка, различимы ли исходы для вызывающего, правильно ли сделан retry — владелец `backend-error-handling-review`; такие места уходят в «Вне scope» с его именем.
...
3. **Цикл виден при загрузке или сборке** — сбой или обход при загрузке модуля (`undefined`, отложенный импорт), запрет компилятора, из-за которого код слили в один пакет, каскадная перекомпиляция при правке одного участника.
